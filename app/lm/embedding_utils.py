...
def _resolve_device(configured_device: str | None) -> str:
    """
    根据当前 PyTorch 环境选择可用设备。
    如果 .env 配了 cuda 但本机 torch 是 CPU 版，自动回退到 cpu，避免导入流程中断。
    """
    device = (configured_device or "cpu").strip() or "cpu"
    ...
    try:
        ...
        if torch.cuda.is_available():
            return device
        logger.warning(f"BGE_DEVICE={device}，但当前 PyTorch 不支持 CUDA，已自动回退到 cpu")
        return "cpu"
    except Exception as exc:
        logger.warning(f"CUDA 可用性检测失败，已自动回退到 cpu：{exc}")
        return "cpu"
...
def get_bge_m3_ef():
    ...
    device = _resolve_device(embedding_config.bge_device)
    use_fp16 = bool(embedding_config.bge_fp16) and device.startswith("cuda")
