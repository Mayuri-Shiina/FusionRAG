...
def step_3_clean_old_data(client, chunks_json_data: List[Dict[str, Any]]):
    """
    步骤3：幂等性处理 - 基于file_title清理旧数据
    核心设计：
        插入新数据前删除同file_title的所有旧切片，确保同一文档多次上传仅保留最新版本。
        这样快速解析版和增强解析版不会在Milvus中同时残留。
    参数：
    ...
    file_titles = sorted({
        ...
        if (title := str(chunk.get("file_title", "")).strip())
    })

    if not file_titles:
        logger.error("Milvus幂等性清理失败：切片中无有效file_title，无法保证同文档覆盖")
        raise ValueError("切片缺失file_title，无法执行Milvus幂等清理")

    if len(file_titles) > 1:
        logger.warning(f"Milvus幂等性清理：本次检测到多个file_title，将逐个清理：{file_titles}")

    for file_title in file_titles:
        _clear_chunks_by_file_title(client, CHUNKS_COLLECTION_NAME, file_title)


def _clear_chunks_by_file_title(client, collection_name: str, file_title: str):
    """
    内部核心函数：根据file_title删除Milvus中的旧切片数据
    参数：
        client - MilvusClient实例
    ...
    if not title:
        logger.warning("Milvus单文档清理跳过：file_title为空")
        return
    if not collection_name:
        logger.warning("Milvus单文档清理跳过：集合名称未配置")
        return

    try:
        # 集合不存在则无需清理
        if not client.has_collection(collection_name=collection_name):
            logger.info(f"Milvus单文档清理跳过：集合{collection_name}不存在")
            return

        # 1. 文件标题安全转义，避免filter表达式报错
        safe_file_title = escape_milvus_string(title)
        filter_expr = f'file_title == "{safe_file_title}"'
        logger.info(f"Milvus幂等性清理：开始删除集合{collection_name}中file_title={title}的旧数据")
        ...
        logger.info(f"Milvus幂等性清理完成：成功删除file_title={title}的旧数据")
    except Exception as e:
        logger.error(f"Milvus幂等性清理失败：file_title={title} | 错误：{str(e)}", exc_info=True)
        raise ValueError(f"幂等清理失败（file_title={title}）: {e}")
