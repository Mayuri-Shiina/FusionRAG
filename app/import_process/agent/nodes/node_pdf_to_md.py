...
def _normalize_fast_pdf_text(text: str) -> str:
    """
    轻量清洗 PyMuPDF 抽取的页面文本。
    目标是保留原文顺序，去掉过多空行，避免影响后续标题切分和向量化。
    """
    if not text:
        return ""
    text = text.replace("\r\n", "\n").replace("\r", "\n")
...
def step_2_fast_extract_pdf_text(pdf_path_obj: Path, output_dir_obj: Path) -> str:
    """
    快速 PDF 解析：用 PyMuPDF 直接抽取可选中文本，生成 Markdown 文件。
    适用：文本型 PDF、对速度优先、无需 OCR/图片摘要/复杂版面恢复的场景。
    不适用：扫描件、图片型 PDF、强依赖表格/图片语义的文档。
    """
    try:
        import fitz
    except ImportError as exc:
        raise RuntimeError("快速 PDF 解析需要 PyMuPDF/fitz，请先安装 pymupdf") from exc

    md_path = output_dir_obj / f"{pdf_path_obj.stem}_fast.md"
    page_sections = [f"# {pdf_path_obj.stem}", "", "> 解析模式：快速解析（仅抽取 PDF 可选中文本，不包含 OCR 与图片理解）。"]

    logger.info(f"[快速PDF解析] 开始抽取文本：{pdf_path_obj}")
    with fitz.open(pdf_path_obj) as document:
        for page_index, page in enumerate(document, start=1):
            page_text = _normalize_fast_pdf_text(page.get_text("text"))
            if not page_text:
                logger.warning(f"[快速PDF解析] 第 {page_index} 页未抽取到文本，可能是扫描件或图片型PDF")
                continue
            page_sections.extend(["", f"## 第 {page_index} 页", "", page_text])

    md_content = "\n".join(page_sections).strip() + "\n"
    if len(md_content.strip()) <= len(page_sections[0]) + 40:
        raise RuntimeError("快速 PDF 解析未抽取到有效文本；该文件可能是扫描件，请使用 MinerU 增强解析")

    md_path.write_text(md_content, encoding="utf-8")
    logger.info(f"[快速PDF解析] Markdown生成成功：{md_path}，内容长度：{len(md_content)}")
...
def node_pdf_to_md(state: ImportGraphState) -> ImportGraphState:
    ...
    try:
        ...
        parse_mode = (state.get("parse_mode") or "auto").strip().lower()
        if parse_mode == "fast":
            logger.info(f"【{func_name}】检测到快速解析模式，跳过 MinerU，使用 PyMuPDF 直接抽取文本")
            md_path = step_2_fast_extract_pdf_text(pdf_path_obj, output_dir_obj)
        else:
