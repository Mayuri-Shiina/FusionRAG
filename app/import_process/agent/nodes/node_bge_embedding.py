...
def step_3_generate_embeddings(texts_to_embed: List[Dict[str, Any]], bge_m3_ef: Any) -> List[Dict[str, Any]]:
    ...
    for i in range(0, total, batch_size):
        ...
        try:
            ...
            # 向量字段是 Milvus 入库的必要字段，失败后继续传递会造成脏状态。
            logger.error(
                f"第{start_idx}-{end_idx}条切片：向量生成失败，已终止入库流程 | 错误原因：{str(e)}",
                exc_info=True
            )
            raise ValueError(f"第{start_idx}-{end_idx}条切片向量生成失败：{e}") from e
